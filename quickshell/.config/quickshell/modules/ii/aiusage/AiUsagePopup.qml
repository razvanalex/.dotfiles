...
PanelWindow {
    ...
    // Anchor item (bar button). Optional: when opened via IPC with the indicator
    // hidden, the popup anchors under the bar's right corner instead.
    ...
    anchors {
        // with an anchorItem follow the ii idiom (anchor under the button);
        // without one (IPC open), hug the bar's right corner instead.
        left: root.anchorItem && (!Config.options.bar.vertical || (Config.options.bar.vertical && !Config.options.bar.bottom))
        right: !root.anchorItem || (Config.options.bar.vertical && Config.options.bar.bottom)
        top: root.anchorItem ? (Config.options.bar.vertical || (!Config.options.bar.vertical && !Config.options.bar.bottom)) : true
        bottom: !root.anchorItem && !Config.options.bar.vertical && Config.options.bar.bottom
    }
        ...
        left: {
            if (!Config.options.bar.vertical) {
                if (!root.anchorItem) {
                    ...
                    // right corner: right edge minus bar + half-card so the card
                    // sits inside the screen with a small gap
                    const m = win ? win.width - (Appearance.sizes.verticalBarWidth + popupBackground.implicitWidth / 2) : 0;
                    return Math.max(8, m);
                }
                const win = root.QsWindow?.window;
