...
export function normalizeEngineState(raw: Partial<WallpaperEngineState>): WallpaperEngineState {
    ...
    const historyCursorRaw = Number.isFinite(raw.historyCursor)
        ...
        : history.length - 1
    const historyCursor = history.length === 0
        ? -1
    ...
    return {
        ...
        intervalSeconds: Number.isFinite(raw.intervalSeconds)
            ? Math.max(MIN_INTERVAL_SECONDS, Math.floor(raw.intervalSeconds as number))
...
export function loadEngineState(path?: string): WallpaperEngineState {
    ...
    try {
        ...
        Logger.error("WallpaperEngine: Failed to load state, using defaults:", error)
        return { ...DEFAULT_ENGINE_STATE }
    }
}
