 ...
 */
function _parseCodeBlock(content: string): { lang: string; code: string } {
...
export function parseMarkdown(content: string): MarkdownBlock[] {
    ...
    while (i < lines.length) {
        ...
        if (isHeading(trimmedLine)) {
            const { level, content: headingContent } =
                parseHeading(trimmedLine);
        ...
        if (isListLine(trimmedLine)) {
            ...
            while (
                i < lines.length &&
                (isListLine(lines[i].trim()) || !lines[i].trim())
            ) {
                if (lines[i].trim()) {
