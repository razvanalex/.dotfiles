...
function ConfigToggle({
    ...
    const isSensitive =
        enabled && typeof enabled === "object" && "as" in enabled
            ? enabled
            : createState(!!enabled)[0];
    ...
    return (
        <button
            ...
            onClicked={() => {
                const current = !!(typeof active === "function"
                    ? active()
                    : ((active as any).get?.() ?? active));
                onChange(!current);
            }}
        >
            <box class="spacing-h-5">
                ...
                <box
                    class={active.as(
                        (e: any) => `switch-bg ${e ? "switch-bg-true" : ""}`,
                    )}
...
function calculateSchemeInitIndex(
...
function ColorSchemeSettings() {
    ...
    return (
        <box
            orientation={Gtk.Orientation.VERTICAL}
            class="osd-colorscheme-settings spacing-v-5 margin-20"
            valign={Gtk.Align.CENTER}
        >
            <box orientation={Gtk.Orientation.VERTICAL}>
                <label
                    halign={Gtk.Align.CENTER}
                    class="txt-norm titlefont txt"
                    label="Options"
                />
                <ConfigToggle
                    icon="dark_mode"
                        ...
                        execAsync([
                            ...
                            .then(() =>
                                execAsync([
                                    "bash",
                                    "-c",
                                    `${configDir}/ags/scripts/color_generation/switchcolor.sh`,
                                ]),
                            )
                            .catch((e) => Logger.error(e));
                    }}
                        ...
                        onChange={(value) => {
                            ...
                            if (value)
                                execAsync([
                                    "bash",
                                    "-c",
                                    `${configDir}/ags/scripts/color_generation/switchcolor.sh - --yes-gradience`,
                                ]).catch((e) => Logger.error(e));
                            else
                                execAsync([
                                    "bash",
            ...
            <box orientation={Gtk.Orientation.VERTICAL} class="spacing-v-5">
                <label
                    halign={Gtk.Align.CENTER}
                    class="txt-norm titlefont txt margin-top-5"
                    label="Scheme styles"
                />
                <ConfigMulipleSelection
                    ...
                    onChange={(value) => {
                        execAsync([
                            "bash",
                            "-c",
                            `mkdir -p ${stateDir}/ags/user && sed -i "3s/.*/${value}/" ${stateDir}/ags/user/colormode.txt`,
                        ])
                            .then(() =>
                                execAsync([
                                    "bash",
...
function ConfigSection({
    ...
    return (
        <box orientation={Gtk.Orientation.VERTICAL} class="spacing-v-5">
            <label
                halign={Gtk.Align.CENTER}
                class="txt txt-large margin-left-10"
                label={name}
            />
            <box
    ...
    return (
        ...
        >
                ...
                >
                    <ConfigSection name="Color scheme">
                        <box class="spacing-h-5" halign={Gtk.Align.CENTER}>
                            <ColorBox
                                name="P"
                                className="osd-color osd-color-primary"
                            />
                            <ColorBox
                                name="S"
                            ...
                            <ColorBox
                                name="E"
                                className="osd-color osd-color-error"
                            />
                        </box>
                        <box class="spacing-h-5" halign={Gtk.Align.CENTER}>
                            <ColorBox
                    ...
                    <ConfigSection name="Effects">
                        ...
                        <Subcategory>
                            <ConfigSpinButton
                                ...
                                initValue={
                                    userOptions.animations.choreographyDelay
                                }
                                minValue={0}
                                maxValue={1000}
                                step={10}
            ...
            <box homogeneous>
                <label
                    halign={Gtk.Align.CENTER}
                    class="txt txt-italic txt-subtext margin-5"
