...
function AppVolume({ stream }: { stream: Wp.Stream }) {
	...
	const _description = createBinding(stream, "description");
	const _name = createBinding(stream, "name");

	// Try to use the icon property from the stream first, fall back to app name
	const baseIconName =
		stream.icon || stream.name?.toLowerCase() || "audio-x-generic-symbolic";
...
function AudioDeviceSelector({ input = false }: { input?: boolean }) {
	...
	const displayText = devices.as((devicesList: any[]) => {
		...
		for (const device of devicesList) {
			const isDefault =
				device.is_default ||
				device.isDefault ||
				(typeof device.get_is_default === "function"
					? device.get_is_default()
					: false);
	...
	return (
		<box
			orientation={Gtk.Orientation.VERTICAL}
			class="sidebar-volmixer-deviceselector"
		>
			<button onClicked={() => setDropdownShown(!dropdownShown.get())}>
				<box class="txt spacing-h-10">
					...
					<label
						class="icon-material txt-norm"
						label={dropdownShown.as((s) => (s ? "expand_less" : "expand_more"))}
					/>
				</box>
			</button>
			<revealer
									...
									>
										<box class="txt spacing-h-10">
											...
											<label
												...
												maxWidthChars={30}
												label={
													device.description || device.name || "Unknown Device"
												}
											/>
											<label
...
export default function AudioControls() {
	...
	if (!wp)
		return (
			<box>
				<label label="WirePlumber not available" />
			</box>
		);
			...
			<box orientation={Gtk.Orientation.VERTICAL} class="spacing-v-5">
				...
				>
					<box class="spacing-h-5">
						...
						<box
							class={isMuted.as(
								(m: boolean) => `switch-bg ${m ? "switch-bg-true" : ""}`,
							)}
							valign={Gtk.Align.CENTER}
						...
						>
							<box
								class={isMuted.as(
									(m: boolean) => `switch-fg ${m ? "switch-fg-true" : ""}`,
								)}
								halign={Gtk.Align.START}
								valign={Gtk.Align.CENTER}
							/>
