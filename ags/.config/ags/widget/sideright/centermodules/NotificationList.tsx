...
const getFriendlyNotifTimeString = (timeObject: number) => {
    ...
    if (oneMinuteAgo && messageTime.compare(oneMinuteAgo) > 0) return "Now";
...
function NotificationIcon({
    ...
    if (notification.appName && icon === "NO_ICON")
        icon = substitute(notification.appName);
    ...
    return (
        ...
        >
            {isIcon ? (
                <image valign={Gtk.Align.CENTER} iconName={icon} />
            ) : (
                <label
                    class="icon-material txt-huger"
                    hexpand
                    label={
                        notification.urgency === Notifd.Urgency.CRITICAL
                            ? "release_alert"
                            : guessMessageType(
                                  notification.summary.toLowerCase(),
                              )
                    }
                />
            )}
...
function NotificationItem({
    ...
    const urgency =
        notification.urgency === Notifd.Urgency.CRITICAL
            ? "critical"
            : notification.urgency === Notifd.Urgency.NORMAL
              ? "normal"
              : "low";
                ...
                <box class="spacing-h-5" hexpand>
                    ...
                    >
                        ...
                        <revealer
                            ...
                            transitionDuration={
                                userOptions.animations.durationSmall
                            }
...
export default function NotificationList() {
    ...
    return (
        ...
        >
            <stack
                visibleChildName={notifications.as((n) =>
            ...
            <box class="txt spacing-h-5">
                <label
                    ...
                    label={notifications.as((n) =>
                        n.length > 0 ? `${n.length} notifications` : "",
                    )}
                />
                <button
                    class={dndState.as(
                        (d) =>
                            `sidebar-centermodules-bottombar-button ${d ? "notif-listaction-btn-enabled" : ""}`,
                    )}
                    onClicked={() => {
