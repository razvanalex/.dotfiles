...
function TimeRow() {
	const uptime = createPoll("", 60000, async () => {
		try {
			const out = await execAsync([
				"bash",
				"-c",
				`uptime -p | sed -e 's/up //;s/ day\\| days/d/;s/ hour\\| hours/h/;s/ minute\\| minutes/m/;s/,[^,]*//2'`,
			]);
			return `Uptime: ${out}`;
		} catch {
	...
	return (
		<box class="spacing-h-10 sidebar-group-invisible-morehorizpad">
			...
			<label class="txt-small txt" label={uptime} halign={Gtk.Align.CENTER} />
			<box hexpand />
			<button
				class="sidebar-iconbutton icon-material txt-norm"
				tooltipText="Reload"
				onClicked={() =>
					execBashAsync("hyprctl reload; killall ags ydotool; ags &")
				}
			>
				<label label="refresh" />
			</button>
...
export default function SideRight(monitor: Gdk.Monitor, index: number = 0) {
	...
	try {
					...
					controller.connect("key-pressed", (_, keyval, _keycode, state) => {
						...
						if (
							checkKeybind(
								keyval,
								state,
								userOptions.keybinds.sidebar.options.nextTab,
							)
						) {
							nextTab();
							return true;
						...
						if (
							checkKeybind(
								keyval,
								state,
								userOptions.keybinds.sidebar.options.prevTab,
							)
						) {
							prevTab();
							return true;
		...
		return (
			<window name={`sideright${index}`} application={app}>
				<label label="Error creating window. Check logs." />
			</window>
		);
	}
}
