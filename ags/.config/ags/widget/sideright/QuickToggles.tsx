...
import { BluetoothIndicator, NetworkIndicator } from "../bar/StatusIcons";
...
export function ToggleIconWifi() {
    ...
    return (
        <ToggleButton
            ...
            tooltipText={wifiEnabled.as((e) =>
                e ? "WiFi | Right-click to configure" : "WiFi disabled",
            )}
            className={wifiEnabled.as(
                (e) =>
                    `txt-small sidebar-iconbutton ${e ? "sidebar-button-active" : ""}`,
            )}
        />
    );
}
...
export function ToggleIconBluetooth() {
    ...
    return (
        <ToggleButton
            ...
            className={isPowered.as(
                (p) =>
                    `txt-small sidebar-iconbutton ${p ? "sidebar-button-active" : ""}`,
            )}
        />
    );
}
...
export function ModuleInvertColors() {
    ...
    return (
        <ToggleButton
            onClick={async () => {
                try {
                    ...
                    if (shader !== "[[EMPTY]]" && shader !== "") {
                        ...
                        await execAsync(
                            `hyprctl keyword decoration:screen_shader ${GLib.get_user_config_dir()}/hypr/shaders/invert.frag`,
                        );
                        setEnabled(true);
                    }
                } catch (e) {
            ...
            child={
                <label class="icon-material txt-norm" label="invert_colors" />
            }
...
export function ModuleIdleInhibitor() {
    ...
    execAsync(`pidof -x wayland-idle-inhibitor.py`)
        .then(() => setEnabled(true))
        .catch(() => setEnabled(false));
    ...
    return (
        <ToggleButton
            onClick={() => {
                if (enabled.get()) {
                    execAsync("pkill -f wayland-idle-inhibitor.py").then(() =>
                        setEnabled(false),
                    );
                } else {
                    execAsync(`python3 ${scriptPath}`).then(() =>
                        setEnabled(true),
                    );
                }
            }}
