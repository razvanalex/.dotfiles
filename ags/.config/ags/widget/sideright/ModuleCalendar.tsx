...
const weekDays = [
    { day: "Mo", today: 0 }, { day: "Tu", today: 0 }, { day: "We", today: 0 },
    { day: "Th", today: 0 }, { day: "Fr", today: 0 }, { day: "Sa", today: 0 },
    { day: "Su", today: 0 },
];

function CalendarDayButton({ day, today }: { day: string | number; today: number; }) {
    return (
        <button class={`sidebar-calendar-btn ${today === 1 ? "sidebar-calendar-btn-today" : today === -1 ? "sidebar-calendar-btn-othermonth" : ""}`}>
            <overlay>
                <box />
                <label halign={Gtk.Align.CENTER} class="txt-smallie txt-semibold sidebar-calendar-btn-txt" label={String(day)} />
            </overlay>
        </button>
    );
}
...
function CalendarWeekDay({ day }: { day: string }) {
    return (
        <label class="sidebar-calendar-btn sidebar-calendar-header-day txt-smallie txt-semibold"
            label={day} halign={Gtk.Align.CENTER} valign={Gtk.Align.CENTER} />
    );
}
...
function CalendarWidget() {
    ...
    return (
        <box orientation={Gtk.Orientation.VERTICAL} class="spacing-v-5" $={(self: Gtk.Box) => {
                const scrollController = new Gtk.EventControllerScroll();
                scrollController.set_flags(Gtk.EventControllerScrollFlags.VERTICAL);
                const scrollId = scrollController.connect("scroll", (_, _dx: number, dy: number) => {
        ...
        >
            <box class="spacing-h-5 sidebar-calendar-header">
                <button class="txt txt-large sidebar-calendar-monthyear-btn" onClicked={() => setMonthShift(0)}
                ...
                <box class="spacing-h-5">
                    <button class="sidebar-calendar-monthshift-btn" onClicked={() => setMonthShift(monthShift.get() - 1)}>
            ...
            <box orientation={Gtk.Orientation.VERTICAL} class="spacing-v-5">
                ...
                <box orientation={Gtk.Orientation.VERTICAL} class="spacing-v-5" $={(self: Gtk.Box) => {
                        const update = (shift: number) => {
                            ...
                            layout.forEach((row) => {
                                const rowBox = (<box class="spacing-h-5" homogeneous>{row.map((day) => (<CalendarDayButton day={day.day} today={day.today} />))}</box>) as unknown as Gtk.Widget;
                                self.append(rowBox);
                            });
                        };
                        const unsub = monthShift.subscribe(() => update(monthShift.get()));
                        onCleanup(unsub);
...
export default function ModuleCalendar() {
    ...
    return (
        <box class="sidebar-group spacing-h-5" vexpand={activeTab.as((t) => t === "todo")}>
            <box valign={Gtk.Align.CENTER} homogeneous orientation={Gtk.Orientation.VERTICAL} class="sidebar-navrail spacing-v-10">
                <button class={activeTab.as((t) => `button-minsize sidebar-navrail-btn txt-small spacing-h-5 ${t === "calendar" ? "sidebar-navrail-btn-active" : ""}`)}
                ...
                <button class={activeTab.as((t) => `button-minsize sidebar-navrail-btn txt-small spacing-h-5 ${t === "todo" ? "sidebar-navrail-btn-active" : ""}`)}
            ...
            <stack visibleChildName={activeTab} transitionType={Gtk.StackTransitionType.SLIDE_UP_DOWN}
                transitionDuration={userOptions.animations.durationLarge} interpolateSize={true}>
                <box $type="named" name="calendar"><CalendarWidget /></box>
                <box $type="named" name="todo"><TodoWidget /></box>
            </stack>
        </box>
    );
}
