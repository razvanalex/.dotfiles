...
export interface CalendarDay {
    day: number;
    today: number; // 1 for today, 0 for current month, -1 for other month
}

export function getCalendarLayout(dateObject: Date | undefined, highlight: boolean): CalendarDay[][] {
    ...
    var calendar: CalendarDay[][] = [...Array(6)].map(() => Array(7));
    var i = 0, j = 0;
    while (i < 6 && j < 7) {
        calendar[i][j] = {
            "day": toFill,
