...
export default function Osd({
    ...
    const [mode, setMode] = createState<"volume" | "brightness" | "microphone">(
        "brightness",
    );
    ...
    if (wp) {
        ...
        const updateAudio = (show = true) => {
            if (currentSpeaker) {
                ...
                const changed =
                    vol !== lastSpeakerVol || mute !== lastSpeakerMute;
        ...
        const connectSpeaker = (speaker: Wp.Endpoint | null) => {
            ...
            if (currentSpeaker) {
                volumeSignal = currentSpeaker.connect("notify::volume", () =>
                    updateAudio(true),
                );
                muteSignal = currentSpeaker.connect("notify::mute", () =>
                    updateAudio(true),
                );
                updateAudio(false);
            }
        ...
        const connectMic = (mic: Wp.Endpoint | null) => {
            ...
            currentMic = mic;
            if (currentMic) {
                micVolumeSignal = currentMic.connect("notify::volume", () =>
                    updateMic(true),
                );
                micMuteSignal = currentMic.connect("notify::mute", () =>
                    updateMic(true),
                );
                updateMic(false);
            }
    ...
    const boxContent = (
        ...
        >
            ...
            >
                <label class="icon-material txt-huger" label={iconName} />
            </button>
            <box orientation={Gtk.Orientation.VERTICAL} spacing={5} hexpand>
                ...
                <slider
                    class={mode.as((m) => {
                        if (m === "brightness")
                            return "osd-progress osd-brightness-progress";
                        if (m === "microphone")
                            return "osd-progress osd-microphone-progress";
