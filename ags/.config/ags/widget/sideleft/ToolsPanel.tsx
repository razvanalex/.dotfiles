...
function ApiKeyInput({
	...
	return (
		<box
			class="sidebar-group spacing-v-5"
		...
		>
			...
			<box class="spacing-h-5">
				...
				<button
					...
					tooltipText={showKey.as((show: boolean) =>
						show ? "Hide key" : "Show key",
					)}
				>
					<label
						label={showKey.as((show: boolean) =>
			...
			<box class="spacing-h-5">
				<button class="txt-norm txt-norm" hexpand onClicked={handleSave}>
					<label label="Save" />
				</button>
				<button class="txt-norm txt-norm" hexpand onClicked={handleClear}>
					<label label="Clear" />
				</button>
			</box>
			<label
				class="txt txt-smallie txt-subtext"
				label={currentKey.as((key: string) =>
					key ? "✓ API key configured" : "✗ No API key set",
				)}
				wrap
			/>
		</box>
...
export default function ToolsPanel() {
	return (
		<box
			orientation={Gtk.Orientation.VERTICAL}
			class="sidebar-tools spacing-v-10"
			vexpand
		>
			<box
				orientation={Gtk.Orientation.VERTICAL}
				class="group-padding spacing-v-10"
				vexpand
			>
				<label class="txt txt-larger" label="API Keys" />
				<label
					class="txt txt-smallie txt-subtext"
					label="Configure your API keys for different AI providers."
					wrap
				/>
				...
				<box orientation={Gtk.Orientation.VERTICAL} class="spacing-v-10">
					...
					<label
						class="txt txt-smallie txt-subtext"
						label="Manage your chat messages and conversation history."
						wrap
					/>
					<box class="spacing-h-5">
						<button
				...
				<box
					orientation={Gtk.Orientation.VERTICAL}
					class="group-padding spacing-v-5"
					vexpand={false}
				>
