...
export default function WallpaperGridView({
		...
		for (let i = 0; i < count; i++) {
			...
			if (item) {
				const shouldBeActive =
					i === selectedIndex && selectedIndex !== Gtk.INVALID_LIST_POSITION;
			...
			if (allMatch) {
				...
				for (let i = 0; i < currentCount; i++) {
					...
					const newPreviewPath = previewLookup
						? previewLookup.get()?.[newItem.id] || newItem.previewPath || ""
						: newItem.previewPath || "";
	...
	return (
		<scrolledwindow
			...
			vscrollbarPolicy={
				useAutoHideScrollbar ? Gtk.PolicyType.AUTOMATIC : Gtk.PolicyType.ALWAYS
			}
