...
const BarResource = (name, icon, command, circprogClassName = 'bar-batt-circprog', textClassName = 'txt-onSurfaceVariant', iconClassName = 'bar-batt') => {
    ...
    const widget = Button({
        ...
        child: Box({
            ...
            setup: (self) => self.poll(1000, () => execAsync(['bash', '-c', command])
                .then((output) => {
    ...
    const trackTitle = Label({
        ...
        setup: (self) => self.hook(Mpris, label => {
            ...
            if (mpris) {
                ...
                if (mpris.trackTitle && artists)
                    label.label = `${trimTrackTitle(mpris.trackTitle)} • ${artists}`;
    ...
    const SystemResourcesOrCustomModule = () => {
        ...
        if (GLib.file_test(CUSTOM_MODULE_CONTENT_SCRIPT, GLib.FileTest.EXISTS)) {
            ...
            child: Box({
                ...
                children: [
                    BarResource('CPU Usage', 'settings_motion_mode', `LANG=C top -bn1 | grep Cpu | sed 's/\\,/\\./g' | awk '{printf("%.2f", $2)}'`,
                    ...
                    BarResource('Disk Usage', 'storage', `LANG=C df -h / | grep -vE '^Filesystem' | awk '{printf("%.2f", $5)}'`,
                        'bar-swap-circprog', 'bar-swap-txt', 'bar-swap-icon'),
                    BarResource('GPU Usage', 'settings_motion_mode', `LANG=C nvidia-smi --query-gpu=utilization.gpu --format=csv,noheader,nounits`,
                        'bar-swap-circprog', 'bar-cpu-txt', 'bar-cpu-icon'),
                    BarResource('GPU Mem Usage', 'memory', `LANG=C nvidia-smi --query-gpu=memory.used,memory.total --format=csv,noheader,nounits | awk '{if ($1 > 0) printf("%.2f\\n", ($1/$2) * 100); else print "0";}'`,
                        'bar-swap-circprog', 'bar-ram-txt', 'bar-ram-icon'),
                    // Revealer({
                    //     revealChild: true,
