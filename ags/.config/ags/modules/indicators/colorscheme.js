...
const sourceEnv = "source ~/.config/ags/scripts/.venv/bin/activate";
const ColorBox = ({
    name = 'Color',
    ...rest
}) => Widget.Box({
...
const ColorSchemeSettingsRevealer = (timeout = 1500) => {
            ...
            children: [
                ...
                ConfigToggle({
                    ...
                    onChange: (self, newValue) => {
                        ...
                        execAsync([`bash`, `-c`, `${sourceEnv} && mkdir -p ${GLib.get_user_state_dir()}/ags/user && sed -i "2s/.*/${transparency}/"  ${GLib.get_user_state_dir()}/ags/user/colormode.txt`])
                            .then(execAsync(['bash', '-c', `${sourceEnv} && ${App.configDir}/scripts/color_generation/switchcolor.sh`]))
                            .catch(print);
                    },
                }),
                Widget.Box({
                    ...
                    children: [
                        ...
                        ConfigMulipleSelection({
                            ...
                            onChange: (value, name) => {
                                ...
                                if (value) execAsync([`bash`, `-c`, `${sourceEnv} && ${App.configDir}/scripts/color_generation/switchcolor.sh - --yes-gradience`, `&`])
                                    .catch(print);
                                else execAsync([`bash`, `-c`, `${sourceEnv} && ${App.configDir}/scripts/color_generation/switchcolor.sh "${ADWAITA_BLUE}" --no-gradience`, `&`])
                                    .catch(print);
        ...
        Widget.Box({
            ...
            children: [
                ...
                ConfigMulipleSelection({
                    ...
                    onChange: (value, name) => {
                        execAsync([`bash`, `-c`, `${sourceEnv} && mkdir -p ${GLib.get_user_state_dir()}/ags/user && sed -i "3s/.*/${value}/" ${GLib.get_user_state_dir()}/ags/user/colormode.txt`])
                            .then(execAsync(['bash', '-c', `${sourceEnv} && ${App.configDir}/scripts/color_generation/switchcolor.sh`]))
                            .catch(print);
                    },
                }),
