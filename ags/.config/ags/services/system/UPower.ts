			...
			if (daemonPart) {
				...
				for (const line of lines) {
					...
					if (trim.startsWith("daemon-version:"))
						...
						this.#daemon.onBattery = trim.substring(11).trim() === "yes";
					else if (trim.startsWith("lid-is-closed:"))
						this.#daemon.lidIsClosed = trim.substring(14).trim() === "yes";
					else if (trim.startsWith("lid-is-present:"))
						this.#daemon.lidIsPresent = trim.substring(15).trim() === "yes";
			...
			for (const block of blocks) {
				...
				for (let i = 1; i < lines.length; i++) {
					...
					const isSectionHeader =
						!isProperty && !isSubProperty && line.match(/^\s{2}[\w\s]+$/);
