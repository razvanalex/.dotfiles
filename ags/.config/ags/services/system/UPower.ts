        ...
        try {
            ...
            if (daemonPart) {
                ...
                for (const line of lines) {
                    ...
                    if (trim.startsWith("daemon-version:"))
                        this.#daemon.version = trim.substring(15).trim();
                    else if (trim.startsWith("on-battery:"))
                        this.#daemon.onBattery =
                            trim.substring(11).trim() === "yes";
                    else if (trim.startsWith("lid-is-closed:"))
                        this.#daemon.lidIsClosed =
                            trim.substring(14).trim() === "yes";
                    else if (trim.startsWith("lid-is-present:"))
                        this.#daemon.lidIsPresent =
                            trim.substring(15).trim() === "yes";
                    else if (trim.startsWith("critical-action:"))
                        this.#daemon.criticalAction = trim.substring(16).trim();
                }
            }
            ...
            for (const block of blocks) {
                ...
                for (let i = 1; i < lines.length; i++) {
                    ...
                    if (isProperty || isSubProperty) {
                        ...
                        else if (key === "power supply")
                            device.powerSupply = val === "yes";
                        else if (key === "updated") device.updated = val;
                        else if (key === "has history")
                            device.hasHistory = val === "yes";
                        else if (key === "has statistics")
                            device.hasStatistics = val === "yes";
                        else if (key === "state") device.state = val;
                        else if (key === "warning-level")
                            device.warningLevel = val;
                        else if (key === "percentage") {
                            const p = parseFloat(val.replace("%", ""));
                            if (!Number.isNaN(p)) device.percentage = p / 100;
                        } else if (key === "icon-name")
                            device.iconName = val.replace(/'/g, "");
                        else if (key === "present")
                            device.present = val === "yes";
                        else if (key === "rechargeable")
                            device.rechargeable = val === "yes";
                    }
                }
                ...
                if (
                    device.path.includes("DisplayDevice") ||
                    device.serial ||
                    device.percentage > -1
                ) {
