...
class SystemService extends GObject.Object {
        ...
        GObject.registerClass({
            Properties: {
                "dark-mode": GObject.ParamSpec.boolean(
                    "dark-mode", "Dark Mode", "Whether dark mode is enabled",
    ...
    private _distroID = execNoExcept(`bash -c 'cat /etc/os-release | grep "^ID=" | cut -d "=" -f 2 | sed "s/\\"//g"'`).trim()
    ...
    get dark_mode() { return this._darkMode }
    ...
    get distroID() { return this._distroID }
    get isDebianDistro() { return ["linuxmint", "ubuntu", "debian", "zorin", "popos", "raspbian", "kali"].includes(this._distroID) }
    get isArchDistro() { return ["arch", "endeavouros", "cachyos"].includes(this._distroID) }
    get hasFlatpak() { return !!execNoExcept(`bash -c 'command -v flatpak'`) }
    get hasPlasmaIntegration() { return !!execNoExcept('bash -c "command -v plasma-browser-integration-host"') }
