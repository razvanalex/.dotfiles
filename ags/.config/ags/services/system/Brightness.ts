...
class Brightness extends GObject.Object {
    ...
    set screen_value(percent: number) {
        ...
        if (this.#useDdc) {
            ...
            execAsync(`brightnessctl -d ${devName} s ${Math.round(percent * 100)}% -q`).catch(e => log.error(e))
            this.#screenValue = percent
            this.notify("screen-value")
            this.emit("changed")
        }
    ...
    async #init() {
        try {
            ...
            const backlights = await execAsync("ls /sys/class/backlight").then(out => out.split("\n").filter(Boolean))
            if (backlights.length > 0) {
                this.#interface = `/sys/class/backlight/${backlights[0]}`
                const max = await execAsync(`cat ${this.#interface}/max_brightness`).then(Number)
                const current = await execAsync(`cat ${this.#interface}/brightness`).then(Number)
