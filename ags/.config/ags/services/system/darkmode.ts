...
import { darkMode, toggleDarkMode } from "./System";
...
function timeInRange(
    ...
    if (timeBefore(rangeStart, rangeEnd)) {
        return (
            timeBeforeOrSame(rangeStart, time) &&
            timeBeforeOrSame(time, rangeEnd)
        );
    } else {
...
export function startAutoDarkModeService() {
    interval(userOptions.time.interval, () => {
        ...
        const fromTime = userOptions.appearance.autoDarkMode.from
            .split(":")
            .map(Number);
        const toTime = userOptions.appearance.autoDarkMode.to
            .split(":")
            .map(Number);
