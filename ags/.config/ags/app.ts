...
app.start({
    ...
    requestHandler(argv: string[], res: (response: any) => void) {
        ...
        const normalizePoolForState = (state: WallpaperEngineState, pool: string[]) => {
            ...
            if (!sameValues || state.queue.length === 0) {
                state.queue = state.strategy === "shuffle"
        ...
        const updateHistory = (state: WallpaperEngineState, wallpaperPath: string) => {
        ...
        const resolveSourcePool = async (state: WallpaperEngineState) => {
            const { wallpaperDir, recursiveSearch, includeHidden } = getDiscoveryOptions()
        ...
        const selectNextWallpaper = async (state: WallpaperEngineState, forwardInHistory: boolean) => {
        ...
        if (argv[0] === "wallpaper") {
            ...
            if (subcommand === "engine") {
                ...
                if (engineCmd === "set") {
                    ...
                    if (key === "interval") {
                        ...
                        if (engineState.isRunning) {
                            wallpaper.updateAutoChangeInterval(interval).catch((error: Error) => {
                                Logger.error("Failed to update running auto mode interval:", error)
                            })
                        }
                    ...
                    if (key === "source") {
                        ...
                        if (!allowed.includes(sourceType)) {
                            return res("error: source must be current-theme|specific-theme|favorites|filtered-library")
                        }
                    ...
                    if (key === "strategy") {
                        ...
                        const allowed: WallpaperStrategy[] = ["random", "shuffle", "sequential"]
                ...
                if (engineCmd === "favorites") {
                    ...
                    if (action === "remove") {
                        ...
                        engineState.favorites = engineState.favorites.filter((item) => item !== path)
                        syncEngineState()
                        return res(JSON.stringify(engineState))
                    }
            ...
            if (subcommand === "random") {
                wallpaper.setRandomWallpaper().then(() => {
