        ...
        const resolveSourcePool = async (state: WallpaperEngineState) => {
            ...
            if (state.sourceType === "specific-theme") {
                if (!state.sourceValue) {
                    throw new Error(
                        "specific-theme source requires a theme path",
                    );
                }
                return findImages(
                    `${wallpaperDir}/${state.sourceValue}`,
                    findOptions,
                );
            }
            ...
            if (subcommand === "engine") {
                ...
                if (engineCmd === "set") {
                    ...
                    if (key === "strategy") {
                        ...
                        if (!allowed.includes(strategy)) {
                            return res(
                                "error: strategy must be random|shuffle|sequential",
                            );
                        }
                ...
                if (engineCmd === "favorites") {
                    ...
                    if (action === "remove") {
                        if (!path)
                            return res(
                                "error: favorites remove requires a path",
                            );
                ...
                if (engineCmd === "recent") {
                    if (argv[3] === "list") {
                        return res(
                            JSON.stringify(
                                engineState.history.slice().reverse(),
                            ),
                        );
                    }
